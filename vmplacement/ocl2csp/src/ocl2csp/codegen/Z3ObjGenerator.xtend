...
//							return resValues
	...
	def getObjectID(EObject obj) {
		...
		if(ids.size != 1){
			throw new RuntimeException("must have one and only one ID")
		}
		val id = ids.head
			...
			if(oclanno != null){
				for (key : oclanno.details.keySet){
					if( (!key.startsWith('Priority_')) &&
						(mutualCons.get(key)==null || !mutualCons.get(key).contains(obj))
					){
						
						val mutFeature = this.mutualConsFeature.get(key)
						try{
							if(mutFeature !=null)
								this.mutualCons.get(key).addAll(obj.eGet(mutFeature) as Collection<EObject>)
						}
						catch(Exception e){}
						try{
							val oclstr = oclanno.details.get(key)
							val query = helper.createQuery(oclstr)
							try{
								val z3expr = query.partialEvaluate(obj, new HashMap<String,Object>());
								val priority = 
										if(key.startsWith('HARD_')) 
											10000 
										else{
											val prioOclStr = oclanno.details.get('Priority_'+key)
											if(prioOclStr.nullOrEmpty)
												100
											else{
												helper.setContext(obj.eClass)
												val prioOclQuery = helper.createQuery(prioOclStr)
												prioOclQuery.fullEvaluate(obj,<String,Object>newHashMap()) as Integer		
											}
											
										}
								if(z3expr == Z3Expr.EMPTY){
									
								}
								else if(z3expr instanceof Z3ConjOfExpr){
									goals.addAll((z3expr as Z3ConjOfExpr).clauses.map[toString])
									weights.addAll((z3expr as Z3ConjOfExpr).clauses.map[priority.toString])
								}
								else{
									goals += z3expr.toString
									weights += priority.toString
								}
							}	
							catch(Exception e){
