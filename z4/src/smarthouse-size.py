...
#        cs.printConfig()
    ...
    def testSolveImprovement(self):
        ...
        for curr in range(0,LOOP):
            ...
            for i in range(0,ROUND):
                #try:
                before = self.violated()
                cs.solve()
                changed = len(cs.changed_para)
                after = self.violated()
                print('{0}-->{1}, with {2}'.format(before,after,changed))
                improved.append((before,after,changed))
               #except NameError:
                    #break
                
                self.contextEvolve(cs.context_value_dict)
                self.configEvolve(cs.config_value_dict)
            print sorted(improved)    

            
    def violated(self):
        cs =self.cs
        for i in range(0,len(cs.config_weight)):
            cs.config_weight[i]+=1000000000
        cs.solve()
        for i in range(0,len(cs.config_weight)):
            cs.config_weight[i]-=1000000000
        return len(cs.results[0].assumps)
