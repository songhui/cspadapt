...
class ContextSolver:
	def __init__(self, config_para, context_para, physical, required, preference):
		...
		for i in self.required:
			self.required_weight.append(random.randint(*GOAL_RANGE))
		self.preference_weight=[]
		for i in self.preference:
			self.preference_weight.append(random.randint(*PREF_RANGE))
		self.config_weight = []
		for i in self.config_para:
			self.config_weight.append(random.randint(*CONF_RANGE))
		#self.calcu_assumps()	
		self.count=0
		self.all_cores = []
		
	def set_context(self, context_value_dict):
		self.context_value_dict = context_value_dict.copy()
		
	def set_config(self,config_value_dict):
		self.config_value_dict = config_value_dict.copy()
