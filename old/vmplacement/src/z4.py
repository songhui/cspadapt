...
class ContextSolver:
	...
	def calcu_assumps(self):
		...
		weights = []	
		breakable = []	
		for i,c in enumerate(self.current_plcs):
			p = Bool(str(len(assumps)))
			assumps.append(p)			
			self.vm_plc_assumpt.append(int(repr(p)))			
			weights.append(self.random.randint(100,500))
			breakable.append(c)	
		
		for i, c in enumerate(self.pm_resource_constraint):
			p = Bool(str(len(assumps))) 
			self.pm_assumpt.append(int(repr(p)))
			assumps.append(p)
			breakable.append(c)
			weights.append(self.random.randint(500000,600000))
...
#			if True:	
				...
				violated_pm = set([int(repr(x)) for x in unsat_core]) & set(self.pm_assumpt)
				print violated_pm
				print self.pm_vm_id
				if len(violated_pm) > 0:
					unsat_core = []
					for p in violated_pm:
						pm_id = self.pm_assumpt.index(p)
						print pm_id
						unsat_core.extend([self.assumps[self.vm_plc_assumpt[vm_id]] for vm_id in self.pm_vm_id[pm_id]])
